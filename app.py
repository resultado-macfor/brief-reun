...
pagina = st.sidebar.radio(
    "Selecione uma opção:",
    options=["📊 Analisar Reunião", "🔑 Gerar Tokens"],
    label_visibility="collapsed"
)

if pagina == "📊 Analisar Reunião":
    st.markdown("## 🎯 Analisador de Reuniões de Vendas")
    st.markdown(
        "**Análise completa de reuniões de vendas** com base em metodologias complexas. "
        "Estruture acordos, tasks e próximos passos automaticamente."
    )
    st.divider()
...
[cole a transcrição completa aqui]""",
        ...
        st.markdown("**Customização (Opcional)**")
        prompt_extra = st.text_input(
            "Instrução adicional:",
            placeholder="Ex: Foque na gestão de objeções e no fechamento",
            help="Direciona a análise para um aspecto específico."
        )
    ...
    if enviar:
        if not transcricao_texto:
            st.warning("⚠️ Por favor, cole a transcrição da reunião para continuar.")
        else:
            with st.spinner("⏳ Analisando com base de conhecimento e estruturando outputs..."):
            ...
            if "Erro" not in resultados["analise_principal"]:
                ...
                with tab2:
                    st.markdown("### 📈 Métricas e Análise Quantitativa")
                    dados_quantitativos = resultados.get("outputs_json", {}).get("analise_quantitativa", {})
                    if dados_quantitativos:
                        criar_dashboard_quantitativo(dados_quantitativos)
                    else:
                        st.info("Nenhuma métrica identificada na análise.")

                with tab3:
                    st.markdown("### 🤝 Acordos e Combinados")
                    st.caption("Acordos verbais identificados na transcrição")
                    ...
                    if acordos:
                        ...
                        st.info("Nenhum acordo específico foi identificado nesta reunião.")

                with tab4:
                    st.markdown("### ✅ Tasks e Responsáveis")
                    st.caption("Tarefas identificadas com responsáveis e prazos")
                    ...
                    if tasks:
                        ...
                        st.info("Nenhuma task específica foi identificada nesta reunião.")

                with tab5:
                    st.markdown("### 📦 Entregáveis Combinados")
                    st.caption("Documentos, propostas e materiais acordados durante a reunião")
                    entregaveis = resultados.get("outputs_json", {}).get("entregaveis", [])
                    if entregaveis:
                        for idx, entregavel in enumerate(entregaveis, 1):
                            with st.container(border=True):
                                display_entregavel_card(entregavel)
                    else:
                        st.info("Nenhum entregável específico foi identificado nesta reunião.")

                with tab6:
                    st.markdown("### ⏭️ Próximos Passos")
                    st.caption("Encaminhamentos e agenda para continuidade")
                    proximos_passos = resultados.get("outputs_json", {}).get("proximos_passos", {})
                    if proximos_passos:
                        st.markdown(proximos_passos)
                    else:
                        st.info("Nenhum próximo passo foi identificado.")
...
{json.dumps(resultados.get("outputs_json", {}), indent=2, ensure_ascii=False)}
...
elif pagina == "🔑 Gerar Tokens":
    st.markdown("## 🔑 Gerar Token para Autenticação API")
    st.markdown("Gere um token JWT para integração com n8n e ferramentas externas. Ele expira após 24h por segurança.")
    st.divider()

    st.markdown("### API REST (para integrações):")
    st.markdown("""
    ...
    api_user = st.text_input("Identificação (User)", value="n8n_integration")

    if st.button("Gerar Token JWT", use_container_width=True):
        token = gerar_token(api_user)
        st.code(token, language="text")
        st.caption("Copie e use no header `Authorization: Bearer <token>`")
